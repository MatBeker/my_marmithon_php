<?php

/**
 * Created by PhpStorm.
 * User: segho_000
 * Date: 24/02/2016
 * Time: 09:32
 */
class galerie
{
    private $bdd ='';

    public function __construct()
    {
        try
        {
            $this->bdd = new PDO('mysql:host=localhost;dbname=bdd_marmiton', 'root', '');
        }
        catch (Exception $my_exception)
        {
            die('Erreur : ' . $my_exception->getMessage());
        }
    }

    public function get_data()
    {
        $donnee_data = $this->bdd->query('SELECT id, name, name_img FROM `recette` ORDER BY date DESC LIMIT 9');

        $data = $donnee_data->fetchAll();
        return ($data);
    }

    public function get_topdata()
    {
        $donnee_topdata = $this->bdd->query('SELECT id,name,rating, name_img FROM `recette` ORDER BY rating DESC LIMIT 3');

        $datatop = $donnee_topdata->fetchAll();
        return ($datatop);
    }
}